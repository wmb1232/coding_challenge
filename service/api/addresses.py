...
def get_address(args, person_id):
    ...
    l = []
    for address_segment in person.address_segments:
        address_seg = address_segment
        l.append(jsonify(AddressSchema().dump(address_seg)))
...
def create_address(payload, person_id):
    ...
    # ahead and create with no additional logic.
        ...
        app.logger.info("William is working")
        person.address_segments[-1].end_date = payload.get("start_date")
        address_segment = AddressSegment(
            street_one=payload.get("street_one"),
            street_two=payload.get("street_two"),
            city=payload.get("city"),
